...
// -------------------------------------------------------------
	function image($atts) 
	{
		...
		$rs = getRow("select * from txp_image where id='$id' limit 1");
		if ($rs) {
			extract($rs);
			$out = array(
				'<img',
...
// -------------------------------------------------------------
	...
	{       
		...
		$rs = getRows("select * from textpattern where Status = 4 and Posted <= now() $catq
				order by $sortq $dirq limit 0,$limit");
...
// -------------------------------------------------------------
	function recent_comments($atts)
	{
		...
		$q = "select txp_discuss.*,textpattern.* from txp_discuss
            left join textpattern on textpattern.ID = txp_discuss.parentid
			order by txp_discuss.posted desc limit 0,$limit";
...
// -------------------------------------------------------------
	function related_articles($atts)
	{
		...
		$cats = getRow("select Category1,Category2 from textpattern where ID='$id' limit 1");  

		$q = array("select * from textpattern where Status = 4 and ID!='$id'",
...
// -------------------------------------------------------------
	function popup($atts) // popup navigation. possible atts: type (c or s), label
	{
		...
		$q[] = "select name from txp_".$thetable." where name != 'default'";
...
// -------------------------------------------------------------
	function category_list($atts) // output href list of site categories
	{
		...
		$label = (empty($label)) ? "" : $label;
		$break = (empty($break)) ? br : $break;
		$q = "select name from txp_category 
				where name != 'default' and type='article' order by name";
		$rs = getThings($q);
		if ($rs) {
			if ($label) $out[] = $label;
			foreach ($rs as $a) {
...
// -------------------------------------------------------------
	function section_list($atts) // output href list of site categories
	{
		...
		$label = (empty($label)) ? "" : $label;
		$break = (empty($break)) ? br : $break;
		$q = "select name from txp_section 
...
// -------------------------------------------------------------
	function paging_link($atts, $thing) 
	{
		global $c,$s,$count,$limit,$uPosted;
				
		if ($c): $filter = " and ((Category1 = '$c') or (Category2 = '$c')) ";
		elseif ($s && $s!='default'): $filter = " and Section = '$s'";
		else: $filter = '';
		endif;

		$nc = getCount('textpattern',
		"Status=4 $filter and (unix_timestamp(Posted) < $uPosted)");	

		if ($nc!=0) {
			if (!empty($thing)) {
				$nextpagelink = "?count=".($count + $limit);
				if ($c) $nextpagelink .= a."c=".urlencode($c);
				return '<a href="'.$nextpagelink.'">'.$thing.'</a>';
			}
		}
		return '';
	}
...
// -------------------------------------------------------------
	...
	{	
		...
		if (is_array($atts)) extract($atts);
		if (is_array($thispage)) { extract($thispage); } else { return; }
...
// -------------------------------------------------------------
	function image_index($atts) 
	{
		...
		$rs = getRows("select * from txp_image 
			where category='$c' and thumbnail=1 order by name");
...
// -------------------------------------------------------------
	function image_display($atts) 
	{
		...
		if($p) {
			$rs = getRow("select * from txp_image where id='$p' limit 1");
			if ($rs) {
				extract($rs);
