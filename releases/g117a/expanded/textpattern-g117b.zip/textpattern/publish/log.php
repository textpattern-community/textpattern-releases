...
// -------------------------------------------------------------
	...
	{	
		...
		$in = doSlash($in);
		extract($in);
		safe_query("insert into txp_log 
			set time=now(),page='$uri',host='$ip',refer='$ref'");
	}
...
// -------------------------------------------------------------
	function mentionInsert($array) 
	{
		...
		if (!$chk) {
			$q = "insert into txp_log_mention set
				article_id = '$id', 
				refpage    = '$refpage', 
				reftitle   = '$reftitle', 
				excerpt    = '$excerpt', 
			...
			safe_query("update txp_log_mention set count=count+1 where refpage='$refpage'");
		}		
	}
