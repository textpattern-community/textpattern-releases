...
#	dmp($pretext);
	...
	if (gps('parentid') && gps('submit')) {
		saveComment();
	} elseif (gps('parentid')) { // is it comments?
...
// -------------------------------------------------------------
	function preText($s,$prefs) 
	{
		...
		$rs = getRow("select * from txp_section where name = '$s' limit 1");
...
// -------------------------------------------------------------
	function textpattern() 
	{
		...
		extract($pretext);
			...
			$html = fetch('user_html','txp_page','name',$page);
...
// -------------------------------------------------------------
	function output_css($s='',$n='')
	{
		if ($n) {
			...
			$cssname = fetch('css','txp_section','name',$s);
		}

		$css = fetch('css','txp_css','name',$cssname);
	...
	function doArticles($atts)
	{
		...
		if ($rs) {
			...
			foreach($rs as $a) {
				...
				$wasAnnotated = (!$Annotate) 
				? getCount('txp_discuss',"parentid=$ID and visible=1") 
				: '';
				...
				$out['thisid']         = $ID;
				$out['posted']         = $uPosted;
				$out['if_comments']    = ($Annotate or $wasAnnotated) ? true : false;
				$out['comments_invite']= ($Annotate or $wasAnnotated)
				...
				$article = doPermlink($article, $out['permlink'], $Title);
...
// -------------------------------------------------------------
	...
	{
		$rs = getThings("select name from txp_section where on_frontpage != '1'");
		if ($rs) {
...
// -------------------------------------------------------------
	function doArticle($atts) 
	{
		...
		$rs = getRow("select *, unix_timestamp(Posted) as uPosted 
				from textpattern where ID='$id' and Status='4' limit 1");
...
// -------------------------------------------------------------
	function article_custom($atts) 
	{
		...
		$rs = getRows("select *, unix_timestamp(Posted) as uPosted from textpattern where 1".
			" and Status=4 and Posted < now() ".
			$category . $section . $excerpted . $month . $author . $keywords .
...
// -------------------------------------------------------------
	...
	{
		$q = array(
			"select ID, Title from textpattern where Posted $type '$Posted'",
...
// -------------------------------------------------------------
	function doPermlink($text, $plink, $Title) 
	{
		global $url_mode;
		$Title = ($url_mode) ? stripSpace($Title) : '';
		return preg_replace("/<(txp:permlink)>(.*)<\/\\1>/sU",
			"<a href=\"".$plink.$Title."\" title=\"".gTxt('permanent_link')."\">$2</a>",$text);
	}

// -------------------------------------------------------------
	function formatDate($uPosted,$count='')
	{
		global $dateformat,$archive_dateformat,$timeoffset,$c,$id;
		
		if ($count > 0 or $id or $c) { $dateformat = $archive_dateformat; }
...
// -------------------------------------------------------------
	...
	{
		global $comments_mode, $url_mode, $pfr;
		$mc = getCount('txp_log_mention',"article_id='$ID'");
		if ($mc) {
			if ($url_mode) {
...
// -------------------------------------------------------------
	function lastMod() 
	{
		$last = getThing('select unix_timestamp(lastmod) from txp_prefs');
...
// -------------------------------------------------------------
	function processTags($matches)
	{
		...
		if ($thing) {
			...
			if (function_exists($tag)) return $tag($atts,$thing);
...
// -------------------------------------------------------------
	function frompath() // Divine what the current article id is, based on the URL 
	{
		...
		if ($pinfo) {
			...
			return (is_numeric($frompath[1])) ? $frompath[1] : '';
		}
		return '';
	}
...
//	Babel that is file permissions. Plugin data is pulled from the db,
//	assembled into a temp PHP file; this is included, then destroyed.

// -------------------------------------------------------------
	function plugins() 
	{
		$rs = getThings("select code from txp_plugin where status=1");
		if ($rs) {
			foreach($rs as $a) { 
				$plugins[] = $a; 
			}
