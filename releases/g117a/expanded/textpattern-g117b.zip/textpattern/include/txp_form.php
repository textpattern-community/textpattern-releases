...
// -------------------------------------------------------------
	function form_list($curname)
	{
		...
		$rs = getRows("select * from txp_form order by name");
...
// -------------------------------------------------------------
	function form_edit($message='')
	{
		...
		if ($step=='form_create') {
			...
			$rs = getRow("select * from txp_form where name='$name'");
			if ($rs) {
				extract($rs);
...
// -------------------------------------------------------------
	function form_save() 
	{
		...
		if ($savenew) {
			safe_query("insert into txp_form set Form='$Form', type='$type',name='$name'");
			form_edit(messenger('form',$name,'created'));
		} else {
			safe_query("update txp_form 
				set Form='$Form',type='$type',name='$name' where name='$oldname'");
...
// -------------------------------------------------------------
	...
	{
		$name = gps('name');
		if ($name) safe_query("delete from txp_form where name='$name'");
