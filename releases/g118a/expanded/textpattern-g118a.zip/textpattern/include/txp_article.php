...
//--------------------------------------------------------------
	...
	{		
		...
		$criteria = ($crit) ? "Title rlike '$crit' or Body rlike '$crit'" : '1';
...
//--------------------------------------------------------------
	...
	//-- textile help --------------
		...
		if ($view=='text') {
			...
			echo graf(
			'<a href="#" onclick="toggleDisplay(\'advanced\');">'.
				gTxt('advanced_options').'</a>'),
			...
			graf(gTxt('article_image').popHelp('article_image').br.
				fInput('text','Image',$Image)),

				// url title
			graf(gTxt('url_title').popHelp('url_title').br.
				fInput('text','url_title',$url_title)).
