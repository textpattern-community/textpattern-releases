...
//-------------------------------------------------------------
	function log_list() 
	{
		...
		safe_query("delete from txp_log where time < date_sub(now(),interval ".
						$txpac['expire_logs_after']." day)");
		safe_query("optimize table txp_log");
		...
		$rs = getRows("select *, unix_timestamp(time) as stamp 
			from txp_log order by time desc limit $offset,$limit");
