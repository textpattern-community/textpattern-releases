...
// -------------------------------------------------------------
	function image_list($message='') 
	{
		...
		$rs = getRows("select * from txp_image order by category,name limit $offset,$limit");
...
// -------------------------------------------------------------
	function image_edit($message='',$id='') 
	{
		...
		$categories[] = '';
		$getcats = getThings("select name from txp_category where type='image'");
		foreach ($getcats as $cat) $categories[$cat] = $cat;
		
		$rs = getRow("select * from txp_image where id='$id'");
				...
				td(
					form(
						graf(gTxt('image_name').br.fInput('text','name',$name)) .
						 graf(gTxt('image_category').br.selectInput('category',
						 		$categories,$category)) .
		...
		if ($extensions[$extension]) {
			...
			if(!$rs){
				...
				if(move_uploaded_file($file, $newpath) == false) {
					safe_query("delete from txp_image where id='$id'");
					safe_query("alter table txp_image auto_increment=$id");
	...
	function thumbnail_insert() 
	{
		...
		if ($extensions[$extension]) {
			...
			if(move_uploaded_file($file, $newpath) == false) {
				...
				chmod($newpath,0755);
				safe_query("update txp_image set thumbnail='1' where id='$id'");
				image_edit(messenger('image',$name,'uploaded'),$id);
			}
...
// -------------------------------------------------------------
	function image_delete() 
	{
		...
		if ($rs) {
			extract($rs);
			$rsd = safe_query("delete from txp_image where id='$id'");
