...
//-------------------------------------------------------------
	function discuss_delete()
	{
		...
		safe_query("delete from txp_discuss where discussid = $discussid");	
...
//-------------------------------------------------------------
	function discuss_list($message='') 
	{
		...
		$criteria = ($crit) ? "where message like '%$crit%'" : ''; 

		$rs = getRows("select *, unix_timestamp(posted) as uPosted 
			from txp_discuss $criteria order by posted desc limit $offset,$limit");
		...
		if($rs) {	
			...
			foreach ($rs as $a) {
				...
				echo tr(
						tda(
						'<a href="index.php?event=discuss'.a.'step=ipban_list">'.
							graf(gTxt('list_banned_ips')),' colspan="1" valign="middle"'
						).
						tdcs(
							form(
...
//-------------------------------------------------------------
	...
	{
		$discussid=gps('discussid');
		extract(getRow("select * from txp_discuss where discussid='$discussid'"));
...
// -------------------------------------------------------------
	function ipban_add() 
	{
		...
		if (!$chk) {
			$rs = safe_query("
				insert into txp_discuss_ipban set
				ip = '$ip',
				name_used = '$name',
...
// -------------------------------------------------------------
	function ipban_unban() 
	{
		...
		$rs = safe_query("delete from txp_discuss_ipban where ip='$ip'");
...
// -------------------------------------------------------------
	function ipban_list($message='')
	{
		...
		$rs = getRows("select * from txp_discuss_ipban order by date_banned desc");
		if ($rs) {
			echo startTable('list'),
			tr(
				hCell('Date banned') .
			...
			foreach($rs as $a) {
				...
				$messagelink = '<a href="?event=discuss'.a.'step=discuss_edit'.a.
					'discussid='.$banned_on_message.'">'.$banned_on_message.'</a>';
				echo
				tr(
					td($datebanned)  .
					td($ip)          .
