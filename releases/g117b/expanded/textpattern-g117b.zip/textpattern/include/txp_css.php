...
//-------------------------------------------------------------
	...
	{	
		...
		$rs = getRows("select name as cssname from txp_css");
		if ($rs) {
			foreach($rs as $a) {
				extract($a);
				...
				$out[] = tr(td($namelink).td($deletelink,10));
			}
			$out[] =  endTable();
			return join('',$out);
		}
		...
		foreach($css as $selector=>$propvals) {
			...
			if(is_array($propvals))
			{
				...
				foreach($propvals as $property=>$value)
				{
					...
					.dLink('css','del_dec','declaration',$i.'-'.$ii,'','name',$name).br;
				}
			$out[] = '</td>'.n.'</tr>';
		 }
		}
...
// -------------------------------------------------------------
	function css_copy() 
	{
		...
		$rs = safe_query("insert into txp_css set css='$css',name='$newname'");
...
// -------------------------------------------------------------
	function css_save_posted() 
	{
		...
		safe_query("update txp_css set css='$css' where name='$name'");
...
//-------------------------------------------------------------
	function css_save() 
	{
		...
		if ($savenew or $copy) {
			safe_query("insert into txp_css set name='$newname', css='$css'");
...
//-------------------------------------------------------------
	function css_delete()
	{
		$name = gps('name');
		if ($name!='default') {
			safe_query("delete from txp_css where name = '$name'");
