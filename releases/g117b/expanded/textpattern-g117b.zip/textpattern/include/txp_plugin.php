...
// -------------------------------------------------------------
	...
	{	
		...
		$rs = getRows("select * from txp_plugin order by name");
		...
		foreach ($rs as $a) {
		  ...
		  tr(
			...
			td($description.' '.pophelp($name),260).
...
// -------------------------------------------------------------
	function switch_status()
	{
		...
		$change = ($status) ? 0 : 1;
		safe_query("update txp_plugin set status=$change where name='$name'");
...
// -------------------------------------------------------------  
	function plugin_save()
	{
		...
		safe_query("update txp_plugin set code='$code' where name='$name'");
...
// -------------------------------------------------------------
	...
	{
		$name = gps('name');
		safe_query("delete from txp_plugin where name='$name'");
	...
	{	
		...
		if(isset($plugin)) {
			...
			if(is_array($plugin)){
				...
				safe_query("insert into txp_plugin set
				name         = '$name',
				status       = 0,
				author       = '$author',
...
// -------------------------------------------------------------
	function plugin_form() 
	{
		...
		hInput('MAX_FILE_SIZE','10000').
		gTxt('install_plugin').': '.
		fInput('file','theplugin','','edit').
		popHelp('install_plugin').'&nbsp;'.
