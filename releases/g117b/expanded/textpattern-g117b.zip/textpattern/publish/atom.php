...
// -------------------------------------------------------------
	function atom() 
	{
		...
		$pub = getRow("select RealName, email from txp_users where privs=1");
		...
		if (!$area or $area=='article') {
			...
			$frs = getThings("select name from txp_section where in_rss != '1'");
			
			foreach($frs as $f) $query[] = "and Section != '".$f."'";
			$query[] = $sfilter;
			$query[] = $cfilter;
	
			$query[] = "and Posted <= now() order by Posted desc limit $limit";
			
			$rs = getRows(join(' ',$query));
			
			foreach ($rs as $a) {
				extract($a);

				if ($txpac['show_comment_count_in_feed']) {
					$dc = getCount('txp_discuss', "parentid=$ID");
					$count = ($dc > 0) ? ' ['.$dc.']' : '';
				} else $count = '';
				...
				$e['subject'] = tag($Category1,'dc:subject');
			...
			extract(gpsa(array('category','limit')));
			$cfilter = ($category) ? "where category='$category'" : '';
			$limit = ($limit) ? $limit : 15;
		
			$query = "select * from txp_link $cfilter order by date desc limit $limit";
			$rs = getRows($query);
