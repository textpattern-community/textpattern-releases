...
// -------------------------------------------------------------
	function search($q)
	{
		...
		$form = fetch('form','txp_form','name','search_results');
			...
			// lost this eventually - only used if search_results form is missing
		$form = (!$form) ? legacy_form() : $form;

		$rs = getRows("
			select ID, Title, Body_html, Section, unix_timestamp(Posted) as uPosted, 
		...
		if($result_rows > 0) {
			foreach($rs as $a) {
				...
				$hurl = ($url_mode)
				?	$siteurl.$path_from_root.$Section.'/'.$ID
...
// -------------------------------------------------------------
	...
	{
		$rs = getThings("select name from txp_section where searchable != '1'");
		if ($rs) {
